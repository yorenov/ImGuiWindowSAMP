...
#include "examples/example_win32_directx11/custom.hpp"
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.frame_color = ImLerp(it_anim->second.frame_color, IsItemActive() ? ImColor(115, 210, 254, 155) : hovered ? ImColor(115, 210, 254, 205) : ImColor(115, 210, 254, 255), g.IO.DeltaTime * 8);

    it_anim->second.Text_color = ImLerp(it_anim->second.Text_color, IsItemActive() ? ImColor(0, 0, 0, 175) : hovered ? ImColor(0, 0, 0, 205) : ImColor(0, 0, 0, 255), g.IO.DeltaTime * 8);


    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, GetColorU32(it_anim->second.frame_color), true, 6.f);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");
    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (size.y - CalcTextSize(label).y) / 2), ImColor(it_anim->second.Text_color), label);
...
bool ImGui::TabButton(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn't be a flag)
        pos.y += window->DC.CurrLineTextBaseOffset - style.FramePadding.y;
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
    const ImRect image_bb(pos, pos + ImVec2(15, 15));
    ...
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, GetColorU32(c::other::tab_button), true, 4.f);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");

    ImGui::GetWindowDrawList()->AddImage(image, image_bb.Min + ImVec2(8, (size_arg.y - 15) / 2), image_bb.Max + ImVec2(8, (size_arg.y - 15) / 2), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::other::ico_tab_button));

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}

bool ImGui::TabIco(const char* label, ImTextureID image, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
   ...
   // RenderFrame(bb.Min, bb.Max, GetColorU32(c::other::tab_button), true, 4.f);
    ...
    ImGui::GetWindowDrawList()->AddImage(image, image_bb.Min + ImVec2(0, (size_arg.y - 30) / 2), image_bb.Max + ImVec2(0, (size_arg.y - 30) / 2), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::other::ico_tab_button));
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    const ImU32 grab_col = ImColor(115,210,254);
    window->DrawList->AddRectFilled(ImVec2(bb_frame.Min.x, bb_frame.Min.y), ImVec2(bb_frame.Max.x - 10, bb_frame.Max.y), bg_col, window->WindowRounding, flags);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
...
bool ImGui::Tab(const ImGuiID id, bool selectable, ImTextureID image, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
	...
	it_anim->second.line = ImLerp(it_anim->second.line, selectable ? ImColor(184, 247, 255, 255) : ImColor(191, 235, 255, 0), g.IO.DeltaTime * 6.f);

	//PushClipRect(bb.Min, ImVec2(bb.Max.x + 120, bb.Max.y + size_arg.y), true);
	ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x + 69,bb.Min.y + 7), ImVec2(bb.Max.x + 43, bb.Max.y - 7),GetColorU32(it_anim->second.line), 4.f, ImDrawFlags_RoundCornersLeft);
	ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 43, bb.Min.y + 7), ImVec2(bb.Max.x - 69, bb.Max.y - 7), GetColorU32(it_anim->second.line), 4.f, ImDrawFlags_RoundCornersRight);
	//PopClipRect();

    it_anim->second.icon = ImLerp(it_anim->second.icon, selectable ? ImColor(115,210,254,255) : ImColor(191, 235, 255, 255), g.IO.DeltaTime * 6.f);
    //it_anim->second.line = ImLerp(it_anim->second.line, selectable ? c::accent_color : c::tab::line_inactive, g.IO.DeltaTime * 6.f);

   // ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(0, size_arg.y - 4), bb.Max, GetColorU32(it_anim->second.line), 30.f, ImDrawCornerFlags_Top);
    ImGui::GetWindowDrawList()->AddImage(image, image_bb.Min + ImVec2(0, (size_arg.y - 30) / 2), image_bb.Max + ImVec2(0, (size_arg.y - 30) / 2), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(it_anim->second.icon));
...
bool ImGui::Keybind(const char* label, int* key, bool show_label)
{
    ...
    const ImGuiID id = window->GetID(label);
    const float width = (GetContentRegionMax().x - style.WindowPadding.x - 5);

    const ImRect rect(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(width, 18.f));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? ImColor(255, 255, 255) : ImColor(64, 64, 64, 255), ImGui::GetIO().DeltaTime * 6.f);
    it_anim->second.slow_show = ImLerp(it_anim->second.slow_show, clickable.Min.x - rect.Min.x, g.IO.DeltaTime * 15.f);

    //window->DrawList->AddRectFilled(ImVec2(it_anim->second.slow_show + rect.Min.x, clickable.Min.y), clickable.Max - ImVec2(0, 0), ImColor(58, 63, 70, 80), 1.f);

    if (show_label) window->DrawList->AddText(rect.Min + ImVec2(0, -5), GetColorU32(it_anim->second.text), label);

    PushStyleColor(ImGuiCol_Text, ImVec4(115 / 255.f, 210 / 255.f, 254 / 255.f, 1.f));
    RenderTextClipped(ImVec2(it_anim->second.slow_show + rect.Min.x, clickable.Min.y - 15), clickable.Max, buf_display, NULL, &label_size, ImVec2(0.5f, 0.5f));
    PopStyleColor();
...
void ImGui::CheckPicker(const char* label, const char* label_picker, bool* v, float col[4])
{
    ...
    SetCursorPos(GetCursorPos() - ImVec2(-3, 32));
    ImGui::ColorEdit4(label_picker, col, ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float square_sz = 24.f, w = (GetContentRegionMax().x - 40 - g.Style.WindowPadding.x) - square_sz;
    // const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(180, 20));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    if (it_anim == anim.end())
    {
        anim.insert({ id, checkbox_state1() });
        it_anim = anim.find(id);
    }

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    it_anim->second.circle_radius = ImLerp(it_anim->second.circle_radius, *v ? 15.f : 0.f, g.IO.DeltaTime * 5);
    it_anim->second.mark_size = ImLerp(it_anim->second.mark_size, *v ? 9.f : 0.f, g.IO.DeltaTime * 5);
    it_anim->second.mark_color = ImLerp(it_anim->second.mark_color, *v ? ImColor(0.f, 0.f, 0.f, 1.f) : ImColor(0.f, 0.f, 0.f, 0.f), g.IO.DeltaTime * 6);
    it_anim->second.Text_color = ImLerp(it_anim->second.Text_color, *v ? ImColor(255, 255, 255, 255) : ImColor(64, 64, 64, 255), g.IO.DeltaTime * 5);
    it_anim->second.check_color = ImLerp(it_anim->second.check_color, *v ? ImColor(115, 210, 254, 255) : ImColor(30, 30, 30, 255), g.IO.DeltaTime * 5);
    it_anim->second.Shadow_color = ImLerp(it_anim->second.Shadow_color, *v ? ImColor(6, 207, 169, 255) : hovered ? ImColor(6, 207, 169, 255) : ImColor(40, 40, 40, 0), g.IO.DeltaTime * 5);
    it_anim->second.check_colors = ImLerp(it_anim->second.check_colors, *v ? ImColor(6, 207, 169, 255) : hovered ? ImColor(0, 0, 0, 0) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 5);
    ...
    window->DrawList->AddRectFilled(total_bb.Min + ImVec2(0, 0), total_bb.Min + ImVec2(16, 16), GetColorU32(it_anim->second.check_color), 1.f);

    RenderCheckMark(window->DrawList, total_bb.Min + ImVec2(8, 8 + it_anim->second.rg) - ImVec2(it_anim->second.mark_size / 2, it_anim->second.mark_size / 2), GetColorU32(it_anim->second.mark_color), it_anim->second.mark_size);

    GetWindowDrawList()->AddText(ImVec2(check_bb.Max.x + 23 - square_sz, check_bb.Min.y - 1), GetColorU32(it_anim->second.Text_color), label);
...
namespace image
{
    extern IDirect3DTexture9* arrow;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w - 8, 26));
    ...
    //GetWindowDrawList()->AddRect(ImVec2(bb.Min.x + 5, bb.Min.y), ImVec2(bb.Max.x - 3, bb.Max.y), GetColorU32(c::combo::background_rect), c::combo::rounding);

    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y - 2), ImVec2(bb.Max.x - 3, bb.Max.y - 2), GetColorU32(c::combo::background_inactive), c::combo::rounding);

    //GetWindowDrawList()->AddRectFilled(ImVec2(value_x2 - 3, bb.Min.y),ImVec2(value_x2 + 25, bb.Max.y), GetColorU32(c::combo::background_inactive), c::combo::rounding);
   // PushFont(sf_pro_text_1);
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    GetWindowDrawList()->AddImage(image::arrow, ImVec2(value_x2, bb.Min.y) + ImVec2(1, 2), bb.Max - ImVec2(9, 6), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::text::text_ico));

    RenderTextClipped(bb.Min + ImVec2(7, 2), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    PopStyleColor();
   // PopFont();
    ...
    ImGui::SetNextWindowSize(ImVec2(bb.GetWidth() - 4, it_anim->second.combo_size));
    ...
    PushStyleColor(ImGuiCol_WindowBg, c::combo::background_inactive);
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (temp_input_is_active)
    {
        ...
        return TempInputScalar(frame_bb, 0, id, label, data_type, p_data, format, is_clamp_input ? p_min : NULL, is_clamp_input ? p_max : NULL);
    }
...
struct slider_state {
    ImVec4 background, circle, text, background_rect;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y), window->DC.CursorPos + ImVec2(w - 11, label_size.y + 17.f));
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? ImColor(39, 39, 39, 255) : ImColor(39, 39, 39, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.background_rect = ImLerp(it_anim->second.background_rect, IsItemActive() || hovered ? ImColor(115, 210, 254, 255) : ImColor(39, 39, 39, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? ImColor(255,255,255) : hovered ? ImColor(255, 255, 255) : ImColor(64, 64, 64), g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 7), frame_bb.Max - ImVec2(0, 4), GetColorU32(it_anim->second.background), 100.f);
    GetWindowDrawList()->AddRect(ImVec2(frame_bb.Min.x + 225, frame_bb.Min.y - 22), ImVec2(frame_bb.Max.x, frame_bb.Max.y - 18), GetColorU32(it_anim->second.background_rect), 2.f);

    if (grab_bb.Max.x > grab_bb.Min.x) {

        it_anim->second.slow_anim = ImLerp(it_anim->second.slow_anim, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 25.f);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 7), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y - 2), GetColorU32(c::slider::background_active), 100.f);

        GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 5, grab_bb.Max.y - 6), 7.5f, GetColorU32(c::slider::circle_active), 100.f);

    }

   // PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    window->DrawList->AddText(ImVec2(total_bb.Min.x - CalcTextSize(value_buf).x / 2 + w - 26, total_bb.Min.y - 20), GetColorU32(it_anim->second.text), value_buf);

    //PopStyleColor();

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w + 11, frame_bb.Min.y - 20), GetColorU32(it_anim->second.text), label);
...
bool ImGui::TempInputText(const ImRect& bb, ImTextureID image, ImGuiID id, const char* label, char* buf, int buf_size, ImGuiInputTextFlags flags)
{
    ...
    bool value_changed = InputTextEx(label,image, NULL, buf, buf_size, bb.GetSize(), flags | ImGuiInputTextFlags_MergedItem);
    if (init)
    {
...
bool ImGui::TempInputScalar(const ImRect& bb, ImTextureID image, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min, const void* p_clamp_max)
{
    char fmt_buf[32];
    char data_buf[32];
    ...
    bool value_changed = false;
    if (TempInputText(bb,image, id, label, data_buf, IM_ARRAYSIZE(data_buf), flags))
    {
        // Backup old value
...
bool ImGui::InputScalar(const char* label, ImTextureID image, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (p_step != NULL)
    {
        ...
        if (InputText("", 0, buf, IM_ARRAYSIZE(buf), flags)) // PushId(label) + "" gives us the expected ID from outside point of view
    ...
    {
        if (InputText(label,image, buf, IM_ARRAYSIZE(buf), flags))
            value_changed = DataTypeApplyFromText(buf, data_type, p_data, format);
    }
    if (value_changed)
        MarkItemEdited(g.LastItemData.ID);
...
bool ImGui::InputScalarN(const char* label, ImTextureID image, ImGuiDataType data_type, void* p_data, int components, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= InputScalar("", 0, data_type, p_data, p_step, p_step_fast, format, flags);
        PopID();
        PopItemWidth();
        p_data = (void*)((char*)p_data + type_size);
    }
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    return InputScalar(label, 0, ImGuiDataType_Float, (void*)v, (void*)(step > 0.0f ? &step : NULL), (void*)(step_fast > 0.0f ? &step_fast : NULL), format, flags);
}

bool ImGui::InputFloat2(const char* label, float v[2], const char* format, ImGuiInputTextFlags flags)
{
    return InputScalarN(label, 0, ImGuiDataType_Float, v, 2, NULL, NULL, format, flags);
}

bool ImGui::InputFloat3(const char* label, float v[3], const char* format, ImGuiInputTextFlags flags)
{
    return InputScalarN(label, 0, ImGuiDataType_Float, v, 3, NULL, NULL, format, flags);
}

bool ImGui::InputFloat4(const char* label, float v[4], const char* format, ImGuiInputTextFlags flags)
{
    return InputScalarN(label, 0, ImGuiDataType_Float, v, 4, NULL, NULL, format, flags);
}
...
bool ImGui::InputInt(const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags flags)
{
    ...
    return InputScalar(label, 0, ImGuiDataType_S32, (void*)v, (void*)(step > 0 ? &step : NULL), (void*)(step_fast > 0 ? &step_fast : NULL), format, flags);
}

bool ImGui::InputInt2(const char* label, int v[2], ImGuiInputTextFlags flags)
{
    return InputScalarN(label,0 , ImGuiDataType_S32, v, 2, NULL, NULL, "%d", flags);
}

bool ImGui::InputInt3(const char* label, int v[3], ImGuiInputTextFlags flags)
{
    return InputScalarN(label, 0, ImGuiDataType_S32, v, 3, NULL, NULL, "%d", flags);
}

bool ImGui::InputInt4(const char* label, int v[4], ImGuiInputTextFlags flags)
{
    return InputScalarN(label, 0, ImGuiDataType_S32, v, 4, NULL, NULL, "%d", flags);
}

bool ImGui::InputDouble(const char* label, double* v, double step, double step_fast, const char* format, ImGuiInputTextFlags flags)
{
    flags |= ImGuiInputTextFlags_CharsScientific;
    return InputScalar(label, 0, ImGuiDataType_Double, (void*)v, (void*)(step > 0.0 ? &step : NULL), (void*)(step_fast > 0.0 ? &step_fast : NULL), format, flags);
}
...
bool ImGui::InputTextMultiline(const char* label, ImTextureID image, char* buf, size_t buf_size, const ImVec2& size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    return InputTextEx(label,image, NULL, buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback, user_data);
}
...
bool ImGui::InputTextEx(const char* label, ImTextureID image, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline)
    {
        ...
        bool child_visible = BeginChildEx(label,image, id, frame_bb.GetSize(), true, ImGuiWindowFlags_NoMove);
        PopStyleVar(3);
        PopStyleColor();
        if (!child_visible)
        {
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state)
{
    ...
    if (BeginChild("undopoints", 0, ImVec2(0.0f, GetTextLineHeight() * 15), true)) // Visualize undo state
    {
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (InputText("##Text", 0, buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
        {
            value_changed = true;
            char* p = buf;
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(225, -2), window->DC.CursorPos + ImVec2(230, -3) + size);
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(-grid_step + off, off), 5.f, ImDrawFlags_RoundCornersRight);
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), 5.f, ImDrawFlags_RoundCornersLeft);
    }
    else
    {
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            RenderFrameBorder(bb.Min, bb.Max, 5.f);
...
{
    ImVec4 text_color, line;
    float text_offset = 5.f, line_offset;
};
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec2 text_min = pos - ImVec2(0, 5);
    const ImVec2 text_max(min_x + size.x, pos.y + size.y);

    // Selectables are meant to be tightly packed together with no click-gap, so we extend their box to cover spacing between selectable.
    ImRect bb(min_x, pos.y, text_max.x, text_max.y);
    ImRect rect(min_x, pos.y, min_x, text_max.y);
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 8.f);
    it_anim->second.text_offset = ImClamp(it_anim->second.text_offset + (6.f * g.IO.DeltaTime * (selected ? 10.f : -5.f)), 4.f, 12.f);


    it_anim->second.line = ImLerp(it_anim->second.line, selected ? c::other::accent_color : c::other::low_color, g.IO.DeltaTime * 6.f);
    it_anim->second.line_offset = ImLerp(it_anim->second.line_offset, selected ? 5.f : 15.f, g.IO.DeltaTime * 6.f);

    ImGui::GetWindowDrawList()->AddRectFilled(rect.Min + ImVec2(0, it_anim->second.line_offset), rect.Max - ImVec2(-3, -it_anim->second.line_offset + 0), GetColorU32(it_anim->second.line), 100.f, ImDrawCornerFlags_Right);
   
    PushStyleColor(ImGuiCol_Text, it_anim->second.text_color);
    RenderTextClipped(text_min + ImVec2(it_anim->second.text_offset, 8), text_max + ImVec2(it_anim->second.text_offset, 0), label, NULL, &label_size, ImVec2(0.0, 0.0), &bb);
    PopStyleColor();
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    BeginChildFrame(id, 0, frame_bb.GetSize());
    return true;
}
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo("##v", NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest,false);
    PopStyleColor(2);

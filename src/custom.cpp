...
void c_custom::tab_area(const char* str_id, ImVec2 size, std::function < void() > content) {
    ...
    BeginChild(std::string(str_id).append(".child").c_str(), 0, ImVec2(value->second.width, size.y));
...
void c_custom::begin_child(const char* name, ImVec2 size) {
    ...
    BeginChild(std::string(name).append(".main").c_str(), 0, size, false, ImGuiWindowFlags_NoScrollbar);
    ...
    SetCursorPosY(30);
    BeginChild(name, 0, { size.x, size.y - 30 });
    SetCursorPosX(10);
